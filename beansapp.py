...
@st.cache_resource
def load_assets():
    """Loads all necessary pickle files for the app."""
    try:
...
if not all([model, scaler, transformer, encoder]):
    ...
    # Define the feature names and their statistical limits.
    # The format has been corrected for better UI experience.
    feature_limits = {
    ...
    # --- FIX APPLIED HERE ---
    # Define the feature names in a list to GUARANTEE the correct order.
    feature_names = [
    ...
    # Create 4 columns for a clean layout
    cols = st.columns(4)

    # Create input fields by looping through the guaranteed-order list
    user_inputs = []
    for i, feature_name in enumerate(feature_names):
        with cols[i % 4]:
            # Look up the limits for the current feature from the dictionary
